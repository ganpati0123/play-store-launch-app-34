...
const HotelPaymentScreen: React.FC = () => {
  const { colors, theme } = useTheme();
  const navigation = useNavigation<NavigationProp>();
  const insets = useSafeAreaInsets();
  const handlePay = () => { Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success); navigation.navigate('HotelConfirmation' as any, { booking: {} as any }); };
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.content}>
        <Text style={[styles.placeholder, { color: colors.text }]}>Hotel Payment</Text>
